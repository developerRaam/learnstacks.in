...
@push('setTitle') Convert PDF to JPG @endpush

@push('addTitle'){{ 'Convert PDF to JPG' }}@endpush
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
        <div class="bg-white rounded-xl p-6">
            ...
            <div class="bg-slate-100 p-8 rounded-2xl border w-full max-w-xl mx-auto">
                <h2 class="text-3xl font-bold mb-6 text-gray-800 text-center">Convert PDF to JPG</h2>
                ...
                <form method="POST" id="submitForm" action="{{ route('tools.convertPdfToJpg') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    <!-- Image Upload -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Choose PDF</label>
                        <input type="file" name="pdf" class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" accept="application/pdf">
                    </div>
                ...
                @if (session('path'))
                    <div class="mt-8 p-6 bg-green-50 border border-green-300 text-green-800 rounded-lg">
                        <div class="flex grid-cols-3 justify-between items-center gap-5">
                            <img src="{{ session('path') }}" id="imagePath" class="w-32 h-auto rounded shadow">
